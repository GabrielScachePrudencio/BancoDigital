...
<p>
	Saldo atual: R$
	<%=String.format("%.2f", usuarioLogado.getSaldo())%></p>
...
<form action="<%=request.getContextPath()%>/pessoa/investir"
	method="post">
	Valor a aplicar:<br> <input type="number" name="valor" step="0.01"
		required><br> <br> Taxa de juros mensal (%):<br>
	<input type="number" name="taxa" step="0.01" required><br>
	<br> Prazo (dias):<br> <input type="number" name="prazoDias"
		required><br> <br>
...
<table border="1" cellpadding="5" cellspacing="0">
	...
	<tr>
		...
		<td><%=inv.getPrazoDias()%></td>
		<td><%=inv.getDataAplicacao().toString().replace("T", " ")%></td>
		<td>R$ <%= String.format("%.2f", inv.getLucroAteAgora()) %></td>

		<td>
			<form action="<%=request.getContextPath()%>/pessoa/resgatar"
