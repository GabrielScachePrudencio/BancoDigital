...
public class InvestirPessoaCommand implements Command {
    ...
    public void execute(HttpServletRequest request, HttpServletResponse response)
        ...
        float taxaMensal = taxaPercent / 100f;
        int prazoDias = Integer.parseInt(request.getParameter("prazoDias"));

        try {
            // aplica investimento (já debita o saldo no banco)
            pessoaDao.investir(usuario.getCpf(), valor, taxaMensal, prazoDias);
