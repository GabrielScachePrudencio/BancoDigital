...
public class InvestimentoDao {
	...
	public Investimento save(Connection conn, Investimento inv) throws SQLException {
		String sql = "INSERT INTO investimento " + "(id_pessoa, valor, taxa_mensal, prazo_dias, data_aplicacao) "
				+ "VALUES (?, ?, ?, ?, ?)";
	...
	public List<Investimento> listarPorPessoa(int idPessoa) throws Exception {
		String sql = "SELECT i.*, p.nome AS nomePessoa " + "FROM investimento i "
				+ "JOIN pessoa p ON p.id = i.id_pessoa " + "WHERE i.id_pessoa = ?";
		...
		try (Connection conn = ConnectionFactory.getConnection(); PreparedStatement ps = conn.prepareStatement(sql)) {
			...
			while (rs.next()) {
				...
				inv.setPrazoDias(rs.getInt("prazo_dias"));
	...
	public Investimento buscarPorId(int id) throws Exception {
	    String sql = "SELECT i.*, p.nome AS nomePessoa " +
	                 "FROM investimento i " +
	    ...
	    try (Connection conn = getConnection();
	        ...
	        if (rs.next()) {
	            Investimento inv = new Investimento();
	            inv.setId(rs.getInt("id"));
	            inv.setIdPessoa(rs.getInt("id_pessoa"));
	            inv.setValor(rs.getFloat("valor"));
	            inv.setTaxaMensal(rs.getFloat("taxa_mensal"));
	            inv.setPrazoDias(rs.getInt("prazo_dias"));
	            inv.setDataAplicacao(rs.getTimestamp("data_aplicacao").toLocalDateTime());
